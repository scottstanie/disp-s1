#!/usr/bin/env python3
# /// script
# dependencies = ["numpy", "dolphin", "pydantic", "tqdm", "tyro"]
# ///
...
def _get_ministack_bounds(
    opera_file_list: Sequence[Path | str],
) -> list[tuple[OperaDispFile, OperaDispFile]]:
    first_last_per_ministack = []
    for d, cur_groupby in itertools.groupby(
        [OperaDispFile.from_filename(fname) for fname in sorted(opera_file_list)],
        key=lambda odf: odf.generation_dt,
    ):
        # cur_groupby is an iterable of all matching
        # Get the first one
        first_file, *_, last_file = cur_groupby
        first_last_per_ministack.append((first_file, last_file))

    return first_last_per_ministack
